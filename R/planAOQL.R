planAOQL <- function (N, pbar, pL, method = c("exact","napprox"), cm=1,intdif = 20) 
{
    ...
    fMinSearch0 = function(nl_, nu_, cm_, nbig_, pbar_, pl_) {
        ...
        if (out_fMS0_ == nu_init_) 
            print("\nupper search interval limit reached")
        if (out_fMS0_ == nl_init_) 
            print("\nlower search interval limit reached")
        return(out_fMS0_)
    }
    ...
    if (method == "napprox") 
        return(new("ACSPlan", n = init_, k = k0AOQL(init_, pL, 
            N)))
    if (method == "exact") {
        fMSmodq2 = function(pl0_, pu0_, n_, k_, nbig_) {
            fMSmodqOpt = function(p_) AOQ(p_, n_, k_, nbig_)
            ...
            if(min(AOQ(pl_init_,n_,k_,nbig_),AOQ(pu_init_,n_,k_,nbig_))==AOQ(seq(pl0_,pu0_,length=500)[p_centre_index_init_],n_,k_,nbig_)) print(
"in fMS: constant objective; unsuitable interval?");
            return(outp$objective)
        }
        kAOQL = function(n_, pl_, nbig_) {
            k1_ = uniroot(function(k_) fMSmodq2(1e-06, 0.3, n_, 
                k_, nbig_) - pl_, c(max(k0AOQL(n_, pl_, nbig_) - 
                0.06, 1.2), 3.2),tol = .Machine$double.eps, maxiter = 5000)$root
            return(k1_)
        }
        fMinSearch = function(nl_, nu_, cm_, nbig_, pbar_, pl_) {
            ImsAOQL = function(n_, cm_, nbig_, pbar_, pl_) {
                kk_ = kAOQL(n_, pl_, nbig_)
            ...
            fMS = function(nl_, nu_, cm_, nbig_, pbar_, pl_) {
                ifelse(nl_ == nu_, nl_, ifelse(ImsAOQL(nl_ + 
                  floor((nu_ - nl_)/2), cm_, nbig_, pbar_, pl_) <= 
                  ImsAOQL(nl_ + floor((nu_ - nl_)/2) + 1, cm_, 
                    nbig_, pbar_, pl_), fMS(floor(nl_), floor(nl_) + 
                  floor((nu_ - nl_)/2), cm_, nbig_, pbar_, pl_), 
                  fMS(floor(nl_) + floor((nu_ - nl_)/2) + 1, 
                    ceiling(nu_), cm_, nbig_, pbar_, pl_)))
            }
        ...
        kfMS = fMinSearch(max(7, init_ - intdif), min(N, init_ + 
            intdif), cm, N, pbar, pL)
        return(new("ACSPlan", n = kfMS, k = kAOQL(kfMS, pL, N)))
    }
  }
